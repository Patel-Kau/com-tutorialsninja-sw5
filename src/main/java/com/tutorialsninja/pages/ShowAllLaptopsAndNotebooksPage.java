package com.tutorialsninja.pages;

import com.aventstack.extentreports.Status;
import com.tutorialsninja.customlisteners.CustomListeners;
import com.tutorialsninja.utility.Utility;
import org.openqa.selenium.By;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.CacheLookup;
import org.openqa.selenium.support.FindBy;
import org.testng.Reporter;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

public class ShowAllLaptopsAndNotebooksPage extends Utility {

    @CacheLookup
    @FindBy(xpath = "//select[@id='input-sort']")
    WebElement shortFilter;
    @CacheLookup
    @FindBy(xpath = "//h1[normalize-space()='MacBook']")
    WebElement productText;
    @CacheLookup
    @FindBy(xpath = "//button[@id='button-cart']")
    WebElement addToCart;
    @CacheLookup
    @FindBy(xpath = "//div[@class='alert alert-success alert-dismissible']")
    WebElement successText;
    @CacheLookup
    @FindBy(xpath = "//a[normalize-space()='shopping cart']")
    WebElement shoppingCartLink;
    @CacheLookup
    @FindBy(xpath = "//h2[normalize-space()='Laptops & Notebooks']")
    WebElement allLaptopsAndNotebooks;


    /**
     * @return
     * @throws InterruptedException
     */
    public List<Double> beforeSortPriceHighToLow() throws InterruptedException {
        List<WebElement> products = driver.findElements(By.xpath("//p[@class ='price']"));
        List<Double> originalProductsPrice = new ArrayList<>();
        for (WebElement e : products) {
            System.out.println(e.getText());
            String[] arr = e.getText().split("Ex Tax:");
            originalProductsPrice.add(Double.valueOf(arr[0].substring(1).replaceAll(",", "")));
        }
        System.out.println(originalProductsPrice);
        // Sort By Reverse order
        Collections.sort(originalProductsPrice, Collections.reverseOrder());
        return originalProductsPrice;
    }

    /**
     * @return
     * @throws InterruptedException
     */
    public List<Double> afterSortPriceHighToLow() throws InterruptedException {
        List<WebElement> products = driver.findElements(By.xpath("//p[@class ='price']"));
        selectByVisibleTextFromDropDown(By.id("input-sort"), "Price (High > Low)");
        // After filter Price (High > Low) Get all the products price and stored into array list
        products = driver.findElements(By.xpath("//p[@class ='price']"));
        ArrayList<Double> afterSortByPrice = new ArrayList<>();
        for (WebElement e : products) {
            String[] arr = e.getText().split("Ex Tax:");
            afterSortByPrice.add(Double.valueOf(arr[0].substring(1).replaceAll(",", "")));
        }
        System.out.println(afterSortByPrice);
        return afterSortByPrice;
    }

    /**
     * shot By Filter
     *
     * @param value
     */
    public void shotByFilter(String value) {
        Reporter.log("Shot By Filter "+ shortFilter.toString());
        CustomListeners.test.log(Status.PASS, "Shot By Filter " + shortFilter);
        selectByVisibleTextFromDropDown(shortFilter, value);
    }

    /**
     * select Product
     *
     * @param product
     */
    public void selectProduct(String product) {
        Reporter.log("Select Product ");
        CustomListeners.test.log(Status.PASS, "select Product");
        clickOnElement(By.xpath("//a[normalize-space()='" + product + "']"));
    }

    /**
     * get MacBook Text
     *
     * @return
     * @throws InterruptedException
     */
    public String getMacBookText() throws InterruptedException {
        Thread.sleep(1000);
        Reporter.log("Get MacBook Text " + productText.toString());
        CustomListeners.test.log(Status.PASS, "Get MacBook Text "+ productText);
        return getTextFromElement(productText);
    }

    /**
     * click On Add To Cart
     */
    public void clickOnAddToCart() {
        Reporter.log("Click On Add To Cart "+ addToCart.toString());
        CustomListeners.test.log(Status.PASS, "Click On Add To Cart "+ addToCart);
        clickOnElement(addToCart);
    }

    /**
     * get Success Text
     *
     * @return
     * @throws InterruptedException
     */
    public String getSuccessText() throws InterruptedException {
        Thread.sleep(1000);
        Reporter.log("Get Success Text "+ successText.toString());
        CustomListeners.test.log(Status.PASS, "Get Success Text "+ successText);
        return getTextFromElement(successText);
    }

    /**
     * click On Shopping CartLink
     *
     * @throws InterruptedException
     */
    public void clickOnShoppingCartLink() throws InterruptedException {
        Thread.sleep(1000);
        Reporter.log("Click On Shopping Cart Link "+ shoppingCartLink.toString());
        CustomListeners.test.log(Status.PASS, "Click On Shopping Cart Link "+shoppingCartLink);
        clickOnElement(shoppingCartLink);
    }

    /**
     * get Laptops And Notebooks Text
     *
     * @return
     * @throws InterruptedException
     */
    public String getLaptopsAndNotebooksText() throws InterruptedException {
        Thread.sleep(1000);
        Reporter.log("Laptops And Notebooks Text "+ allLaptopsAndNotebooks.toString());
        CustomListeners.test.log(Status.PASS, "Laptops And Notebooks Text "+ allLaptopsAndNotebooks);
        return getTextFromElement(allLaptopsAndNotebooks);
    }
}
